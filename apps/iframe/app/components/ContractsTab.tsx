...
function isContractClosed(status: ContractStatus): boolean {
  return getStatusPriority(status) === 4;
}

// Memoized individual contract card component
const ContractCard = memo(
  ({ contract: initialContract }: { contract: Contract }) => {
    const navigate = useNavigate();
    const [contract, setContract] = useState(initialContract);
    const [isFlashing, setIsFlashing] = useState(false);

    // Poll for updates to this specific contract every 3 seconds (skip if closed)
    useEffect(() => {
      // Don't poll closed contracts
...
export function ContractsTab({ user }: ContractsTabProps) {
  ...
  const displayContracts =
    contracts.length > 0 ? contracts : contractsState.value?.data || [];
