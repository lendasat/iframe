...
import { Box, Button, Callout, Dialog, Flex, Heading, Text } from "@radix-ui/themes";
...
export function ContractPendingKyc({ contractId, kycLink, isKycDone }: ContractPendingKycProps) {
  ...
  return (
    <Box className="flex flex-col space-y-4">
      ...
      <Text className={"mb-3 text-font dark:text-font-dark"} weight={"medium"} size={"3"}>
        Follow this <a className="text-blue-500 hover:underline" href={kycLink}>link</a> to start the KYC process.
      </Text>
      <Text className={"mb-3 text-font dark:text-font-dark"} weight={"medium"} size={"3"}>
        The status of the KYC process is{" "}
        <strong>
          <em>
            {isKycDone ? <Text className="text-green-500">DONE</Text> : <Text className="text-gray-500">PENDING</Text>}
          </em>
        </strong>.
