...
import { Box, Tabs, Button, Heading, Flex, Avatar, Text, Spinner, Callout, Separator, } from "@radix-ui/themes";
            ...
            <Tabs.List
              ...
              className="border-b-0 shadow-none md:flex-col rounded-r-full md:rounded-none">
              {
                ['profile', 'security'].map((items, index) => (
                  <Tabs.Trigger
                    key={index}
                    className={`md:justify-start data-[state=active]:before:bg-transparent flex-grow md:w-fit px-2 rounded-full */hover:bg-transparent ${items === 'delete account' ? 'text-red-600 data-[state=active]:bg-red-600/20 md:mt-8' : 'data-[state=inactive]:text-font/70 data-[state=active]:text-purple-800 data-[state=active]:bg-purple-800/20'} font-medium data-[state=active]:font-semibold capitalize`}
                    value={items}
                  >
                    {items}
                  </Tabs.Trigger>
                      ...
                      <Box p={'4'} className="border border-purple-400/20 rounded-2xl">
                        <Flex align={'center'} gap={'3'}>
                          ...
                          <Flex align={'start'} direction={'column'} gap={'1'}>
                            ...
                            <Text size={'2'} className="text-font/50">{new Date(user.created_at).toLocaleDateString('en-CA', options)}</Text>
                            {
                              user.verified && (
                                <Flex gap={'1'}>
                          ...
                          <Box>
                            <Flex direction={'column'} gap={'1'}>
                              <Text as="label" weight={'medium'} size={'2'}
                                className="text-font/50">
                                Full Name</Text>
                              <Text size={'3'} weight={'medium'} className="capitalize text-font-dark">{user.name}</Text>
                            </Flex>
                          </Box>
                          ...
                          <Box>
                            <Flex direction={'column'} gap={'1'}>
                              <Text as="label" weight={'medium'} size={'2'}
                                className="text-font/50">
                                Email Address</Text>
                              <Text size={'3'} weight={'medium'} className="capitalize text-font-dark">{user.email}</Text>
                            </Flex>
                          </Box>
                          <Box>
                            <Flex direction={'column'} gap={'1'}>
                          ...
                          <Box>
                            <Flex direction={'column'} gap={'1'}>
                              <Text as="label" weight={'medium'} size={'2'}
                                className="text-font/50">
                                Joined on</Text>
                              <Text size={'3'} weight={'medium'} className="capitalize text-font-dark">{new Date(user.created_at).toLocaleDateString('en-CA', options)}</Text>
                            </Flex>
                          </Box>
                        </Box>
                      </Box>
