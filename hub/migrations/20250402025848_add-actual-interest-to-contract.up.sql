...
-- Add interest column to contracts table
ALTER TABLE contracts
    ADD COLUMN interest DECIMAL NOT NULL DEFAULT 0;

-- Calculate interest for existing contracts based on interest_rate and duration_days
-- The calculation: interest = loan_amount * interest_rate * (duration_days / 360)
-- This assumes interest_rate is stored as a decimal (e.g., 0.05 for 5%)
UPDATE contracts
SET interest = ROUND(loan_amount * (interest_rate / 360) * duration_days::decimal, 2);


-- Remove default constraint once data is populated
ALTER TABLE contracts
    ALTER COLUMN interest DROP DEFAULT;
