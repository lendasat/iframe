ALTER
TYPE loan_asset_chain
    ADD
    VALUE IF NOT EXISTS 'Solana';


CREATE
TYPE sideshift_solana_network AS ENUM ('mainnet');

-- We can drop these because this feature has not been enabled yet
DROP TABLE IF EXISTS sideshift_shifts;
DROP TABLE IF EXISTS sideshift_quotes;

-- Drop the existing type
DROP
TYPE sideshift_network;

-- Create the updated composite type
CREATE
TYPE sideshift_network AS (
    network_type TEXT, -- 'ethereum', 'bitcoin', or 'solana'
    ethereum_network sideshift_ethereum_network,
    bitcoin_network sideshift_bitcoin_network,
    solana_network sideshift_solana_network
);

-- Recreate the quotes table with updated constraints
...
(
    ...
    -- Updated constraints to include Solana network
    CONSTRAINT valid_deposit_network CHECK (
            ((deposit_network).network_type = 'ethereum' AND
                ...
                (deposit_network).bitcoin_network IS NULL AND
                (deposit_network).solana_network IS NULL) OR
            ((deposit_network).network_type = 'bitcoin' AND
                (deposit_network).bitcoin_network IS NOT NULL AND
                (deposit_network).ethereum_network IS NULL AND
                (deposit_network).solana_network IS NULL) OR
            ((deposit_network).network_type = 'solana' AND
                (deposit_network).solana_network IS NOT NULL AND
                (deposit_network).ethereum_network IS NULL AND
                (deposit_network).bitcoin_network IS NULL)
        ),
    CONSTRAINT valid_settle_network CHECK (
            ((settle_network).network_type = 'ethereum' AND
                ...
                (settle_network).bitcoin_network IS NULL AND
                (settle_network).solana_network IS NULL) OR
            ((settle_network).network_type = 'bitcoin' AND
                (settle_network).bitcoin_network IS NOT NULL AND
                (settle_network).ethereum_network IS NULL AND
                (settle_network).solana_network IS NULL) OR
            ((settle_network).network_type = 'solana' AND
                (settle_network).solana_network IS NOT NULL AND
                (settle_network).ethereum_network IS NULL AND
                (settle_network).bitcoin_network IS NULL)
        )
);
