CREATE
TYPE sideshift_coin AS ENUM ('usdc', 'usdt', 'btc');

CREATE
TYPE sideshift_bitcoin_network AS ENUM ('bitcoin');
CREATE
TYPE sideshift_ethereum_network AS ENUM ('mainnet', 'arbitrum', 'polygon');

-- Create a composite type for network to handle the enum variants
CREATE
...
(
    ...
    -- Add constraints to ensure only one network type is set. Otherwise invalid network combinations might get accepted
...
-- useful to efficiently delete expired quotes
CREATE INDEX idx_sideshift_quotes_expires_at ON sideshift_quotes (expires_at);

CREATE
TYPE sideshift_shift_kind AS ENUM ('fixed', 'variable');
CREATE
TYPE sideshift_shift_status AS ENUM (
'waiting',
'pending',
'processing',
'review',
