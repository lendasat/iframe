...
async fn post_contract_request(
    ...
    let (borrower_loan_address, moon_invoice, fiat_loan_details) = match body.loan_type {
        ...
        LoanType::PayWithMoon | LoanType::MoonCardInstant => {
            ...
            if body.loan_type == LoanType::PayWithMoon && offer.loan_payout != LoanPayout::Direct {
                return Err(Error::InvalidPayWithMoonPayout {
                    actual_payout: offer.loan_payout,
                });
            }

            if body.loan_type == LoanType::MoonCardInstant
                && offer.loan_payout != LoanPayout::MoonCardInstant
            {
    ...
    let installments = {
        let (interest_rate, late_penalty) = match offer.loan_payout {
            LoanPayout::Direct | LoanPayout::Indirect => {
                (offer.interest_rate, LatePenalty::FullLiquidation)
            }
            // These loans are 0-interest if they are repaid before 30 days pass.
            LoanPayout::MoonCardInstant => (Decimal::ZERO, LatePenalty::InstallmentRestructure),
        };
...
async fn cancel_contract_request(
    ...
    if contract.status != model::ContractStatus::Requested
        && contract.status != model::ContractStatus::Approved
    {
...
async fn get_contracts(
    ...
    let total = if statuses.is_empty() {
        ...
        db::contracts::count_contracts_by_borrower_id_and_statuses(&data.db, &user.id, &statuses)
            .await
            .map_err(Error::database)?
    };

    let contracts = if statuses.is_empty() {
...
async fn put_installment_paid(
    ...
    if contract.status != model::ContractStatus::PrincipalGiven {
...
async fn get_recover_collateral_psbt(
    ...
    if contract.status != model::ContractStatus::CollateralRecoverable {
...
async fn post_claim_tx(
    ...
    let claim_amount = signed_claim_tx
        .output
        .iter()
        .map(|o| o.value)
        .sum::<Amount>();
    let claim_amount = claim_amount.to_sat();
    ...
    db::contracts::mark_contract_as_closing_by_claim(&data.db, &contract_id)
        .await
        .map_err(Error::database)?;
...
async fn post_recover_tx(
    ...
    let claim_amount = signed_recovery_tx
        .output
        .iter()
        .map(|o| o.value)
        .sum::<Amount>();
    ...
    data.btsieve
        .send(btsieve::PostTx(signed_recovery_tx))
        .await
        .expect("actor to be alive")
        .map_err(Error::post_claim_tx)?;
    ...
    let rest_of_timeline = event_logs
        ...
        .map(|log| {
            let txid = match log.new_status {
                ...
                | model::ContractStatus::Defaulted => transactions.iter().find_map(|tx| {
